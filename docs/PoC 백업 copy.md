...
| 구분 | 범위 |
|---|---|
| 필수 구현 범위 | `/analyze` 기반 로그 수집, 정규화, fingerprint 생성, Known/New Pattern 판정, PatternOps match, anomaly 탐지, scenario evidence 통합, RAG 조회 |
| 필수 검증 범위 | LangGraph Agent 라우팅, `SharedState` evidence 누적, Recommendation JSON 품질 게이트, fallback, accepted normal 재탐지 |
| 제외 범위 | trajectory model의 정식 학습/예측 모델화, 장기 시계열 기반 장애 진행 예측, 운영 자동 조치 실행 |
| 후속 확장 범위 | trajectory feature 고도화, 서비스별 장애 진행 경로 모델링, 유사 trajectory 기반 사전 위험 예측, Source Code Analysis Agent 연계 |
...
| 이슈 구분 | 문제 상황 및 원인 | 참고한 기준 또는 비교 관점 | 최종 선택 및 적용 이유 |
|---|---|---|---|
| 워크플로우 | 단순 순차 함수 호출만으로는 Agent별 실행 상태, 조건부 skip, 실패 및 재시도를 추적하기 어려웠다. | Agentic Workflow의 Multi-Agent, Meta-Controller, Blackboard 패턴과 LangGraph `StateGraph` 구조를 비교했다. | LangGraph + `OrchestratorAgent` + `SharedState`를 선택했다. Agent 간 직접 호출을 줄이고, 상태 기반 라우팅과 실패 기록을 일관되게 남길 수 있기 때문이다. |
| 프롬프트/품질 | LLM 권고가 일반론에 머물거나 근거 없는 원인과 위험한 운영 조치를 포함할 수 있었다. | 구조화 JSON 출력, PEV, Reflection Loop, hard-fail gate를 비교했다. | Recommendation schema, 100점 rubric, 80점 threshold, 최대 3회 재생성, fallback을 적용했다. 권고 품질을 자동 검증하고 금지 조치를 차단하기 위해서다. |
| 도구 연동 | API 입력과 내부 tool `arguments`의 타입/필수값이 섞이면 런타임 포맷 오류가 발생하기 쉬웠다. | FastAPI/Pydantic 모델 검증과 tool registry 경계 분리를 비교했다. | 외부 입력은 Pydantic request model로 검증하고, Agent 내부 호출은 in-process MCP-style registry로 통일했다. API 계약과 내부 adapter 책임을 분리하기 위해서다. |
| RAG 검색 | Pattern Template, Knowledge Card, Known Pattern, Incident Summary를 한 collection에 넣으면 metadata와 검색 결과가 섞일 수 있었다. | 목적별 collection과 단일 collection + metadata filtering 방식을 비교했다. | `pattern_templates_v2`, `case_cards_v2`, `known_patterns_v2`, `incident_summaries_v2`로 분리했다. 검색 목적과 embedding text가 다르기 때문이다. |
| 임베딩 성능 | 모든 문서에 큰 embedding 차원을 고정하면 짧은 pattern template에도 저장 공간과 호출 비용이 커진다. | 문서 유형별 검색 품질과 저장 비용을 비교했다. | pattern embedding은 기본 1024차원, case/knowledge 문서는 기본 1536차원으로 분리했다. 짧은 패턴 검색은 비용을 줄이고, 긴 지식 문서는 표현력을 유지하기 위해서다. |
| 패턴 분산 | request ID, 숫자, URL, 파일 경로, timestamp 같은 volatile 값 때문에 동일 장애가 여러 fingerprint로 갈라졌다. | 정규화 rule, duplicate candidate, manual merge 방식을 비교했다. | deterministic normalization, pattern rule suggestion, duplicate merge를 조합했다. 동일 구조 로그를 같은 fingerprint 계열로 묶기 위해서다. |
| 정상 피드백 | 운영자가 정상으로 승인한 반복 패턴을 exception처럼 숨기면 관측 가능성이 사라지고, 이후 임계치 초과도 탐지할 수 없었다. | exception suppression과 accepted normal baseline을 비교했다. | accepted normal은 목록에 유지하되 anomaly에서 제외하고, 허용치를 넘으면 breach로 재탐지하도록 했다. 정상 승인 이후에도 관측성과 재탐지 가능성을 유지하기 위해서다. |
| 성능/기타 | 회귀 테스트에서 timezone 없는 `datetime.utcnow()` 사용과 TestClient/httpx 호환성 deprecation warning이 발생한다. | 기능 실패 여부와 후속 정리 필요성을 분리해 확인했다. | 대표 테스트는 통과했고 warning은 후속 개선으로 분리했다. 현재 PoC 기능 검증에는 영향이 없지만, `datetime.now(datetime.UTC)` 전환과 테스트 client 의존성 정리가 필요하다. |

### PoC 성공 기준

| 검증 항목 | 성공 기준 |
|---|---|
| Agent 라우팅 | `/analyze` 결과에서 `LogCollectorAgent`, `LogAnalysisAgent`, `AnomalyDetectionAgent`, `KnowledgeBaseRAGAgent` 실행 이력이 확인된다. |
| 기본 분석 응답 | HTTP 200을 반환하고 `final.evidence_bundle`에 summary, PatternOps, cluster, trajectory 관련 evidence가 포함된다. |
| Recommendation 분리 | 기본 `/analyze`에서는 `RecommendationAgent`가 skipped로 기록되고, 선택 fingerprint API에서만 상세 권고를 생성한다. |
| 권고 JSON 구조 | `executive_summary`, `root_cause_analysis`, `recommended_actions`, `verification_steps`, `prevention_steps`, `confidence`를 포함한다. |
| 품질 게이트 | quality score 80점 이상이면 통과하고, 미달 시 최대 3회 재시도한다. |
| Fallback | JSON 파싱 또는 평가 실패 시 deterministic fallback을 반환하고 `decisions.assumptions`에 사유를 기록한다. |
| 저장 정책 | 권고는 자동 저장하지 않고 `POST /recommendations/save` 호출 시에만 recommendation history에 저장한다. |
| Accepted Normal | 정상 승인된 fingerprint는 anomaly에서 제외되지만, 허용 count를 초과하면 `ACCEPTED_NORMAL_BREACH`로 다시 탐지된다. |
...
핵심 동작은 다음 세 가지 경로로 재현할 수 있다.

| 확인 대상 | 재현 방법 | 기대 결과 |
|---|---|---|
| 기본 분석 workflow | `POST /analyze`에 `service_name`, `goal`, `analysis_date`, `save_to_chromadb=false`를 전달한다. | Agent routing, scenario evidence, PatternOps summary, RAG 조회 결과가 포함된 `AnalyzeResponse`를 받는다. |
| 선택 fingerprint 권고 | `/analyze` 결과 또는 fingerprint 목록에서 하나를 선택해 `POST /recommendations/fingerprint`를 호출한다. | 구조화 권고 JSON, quality score/status, evidence bundle이 생성되며 자동 저장은 되지 않는다. |
| 대표 회귀 테스트 | `cd LOG_DETECT_AGENTS_BACK` 후 `pytest app/tests/test_graph_e2e.py tests/test_recommendation_agent.py tests/test_accepted_normal_patterns.py -q`를 실행한다. | Agent routing, Recommendation quality gate, accepted normal 동작이 모두 통과한다. |
