...
SERVICE_CRITICALITY = {"login-service": "HIGH", "board-service": "MEDIUM", "batch-service": "LOW"}
...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    existing_columns = [row[1] for row in cur.execute("PRAGMA table_info(impact_evaluations)").fetchall()]
...
def recommendation_for(conn: sqlite3.Connection, fp: str, sub_category: str) -> dict[str, str]:
    ...
    cur = conn.cursor()
    row = cur.execute(
        "SELECT cause, recommendation, confidence FROM knowledge_cards WHERE fingerprint=? ORDER BY created_at DESC LIMIT 1", (fp,)
    ).fetchone()
    if row:
    ...
    rules = {
        ...
        "IOException": ("파일 또는 네트워크 I/O 오류", "경로 권한과 재시도 로직을 점검"),
        "DB Timeout": ("DB 응답 지연", "슬로우 쿼리와 커넥션 풀을 점검"),
        "HTTP Timeout": ("외부 HTTP 응답 지연", "타임아웃/재시도/서킷브레이커 설정 점검"),
    }
...
def run_detection_pipeline(service_name: str | None = None) -> dict[str, Any]:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        rows = cur.execute(f"SELECT service_name, level, message, COALESCE(stack_trace,''), created_at FROM service_logs {where}", params).fetchall()
        groups: dict[str, dict[str, Any]] = {}
        for svc, level, msg, stack, created in rows:
            fp = fingerprint_id(svc, level.upper(), msg, stack)
            item = groups.setdefault(fp, {"fingerprint": fp, "occurrence_count": 0, "log_level": level.upper(), "message": msg, "stacktrace": normalize_stacktrace(stack), "service_name": svc, "first_seen": created, "last_seen": created})
            item["occurrence_count"] += 1
            item["first_seen"] = min(item["first_seen"], created)
            item["last_seen"] = max(item["last_seen"], created)
        for g in groups.values():
            cur.execute("REPLACE INTO fingerprints VALUES (:fingerprint,:occurrence_count,:log_level,:message,:stacktrace,:service_name,:first_seen,:last_seen)", g)
        ignored = {r[0] for r in cur.execute("SELECT fingerprint FROM exception_registry").fetchall()}
        anomalies = []
        impacts = []
        recs = []
        for fp, g in groups.items():
            cat, sub = classify(g["message"], g["stacktrace"])
            known = cur.execute("SELECT 1 FROM known_patterns WHERE fingerprint=? OR sub_category=? LIMIT 1", (fp, sub)).fetchone() is not None
            cur.execute("REPLACE INTO log_analysis_results(fingerprint,category,sub_category,is_known_pattern,is_new_pattern) VALUES (?,?,?,?,?)", (fp, cat, sub, int(known), int(not known)))
            baseline = max(1, math.ceil(g["occurrence_count"] / (3 if g["occurrence_count"] >= 25 else 1)))
            ...
            cur.execute("REPLACE INTO anomaly_results VALUES (?,?,?,?,?,CURRENT_TIMESTAMP)", (fp, int(anomaly), spike_ratio if anomaly else 0, severity if anomaly else "NONE", "NEW_ERROR" if not known else "SPIKE"))
            if anomaly:
                anomalies.append({"system": g["service_name"], "severity": severity, "pattern": fp, "message": g["message"], "spike_ratio": spike_ratio})
            score = min(100, LEVEL_SCORE.get(g["log_level"], 5) + CRITICALITY_SCORE.get(SERVICE_CRITICALITY.get(g["service_name"], "LOW"), 5) + min(20, g["occurrence_count"] // 5))
            if fp in ignored:
                score = 0
            level = risk_level(score)
            cur.execute("REPLACE INTO impact_evaluations(fingerprint,risk_score,risk_level,detected,created_at) VALUES (?,?,?,?,CURRENT_TIMESTAMP)", (fp, score, level, int(score > 0)))
            impacts.append({"fingerprint": fp, "risk_score": score, "risk_level": level, "detected": score > 0})
            r = recommendation_for(conn, fp, sub)
            r.update({"fingerprint": fp, "sub_category": sub})
            recs.append(r)
        conn.commit()
        total_logs = len(rows)
        known_count = cur.execute("SELECT COUNT(*) FROM log_analysis_results WHERE is_known_pattern=1").fetchone()[0]
        new_count = cur.execute("SELECT COUNT(*) FROM log_analysis_results WHERE is_new_pattern=1").fetchone()[0]
...
def register_exception(fp: str, reason: str) -> None:
    ...
    with sqlite3.connect(_resolve_db_path()) as conn:
        ensure_schema(conn)
        conn.execute("REPLACE INTO exception_registry(fingerprint, reason) VALUES (?, ?)", (fp, reason))
        conn.commit()


def approve_result(fp: str, cause: str, recommendation: str, action: str, confidence: str) -> str:
    """Persist an approved AI/rule result as a reusable Knowledge Card."""
    card_id = "KC-" + datetime.utcnow().strftime("%H%M%S%f")[:9]
    with sqlite3.connect(_resolve_db_path()) as conn:
        ensure_schema(conn)
        conn.execute("INSERT INTO knowledge_cards(card_id,fingerprint,cause,recommendation,action,confidence) VALUES (?,?,?,?,?,?)", (card_id, fp, cause, recommendation, action, confidence))
        conn.commit()
    return card_id
